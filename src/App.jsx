...
const StaffDirectory = () => {
  ...
  const load = async () => {
    setLoading(true);
    try { setStaff(await staffService.getAll()); }
    catch { setAlert({ type: "error", msg: "Could not load staff records." }); }
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = { full_name: form.full_name, phone: form.phone || null, role: form.role, staff_type: form.staff_type, monthly_salary: form.staff_type === "permanent" ? parseFloat(form.monthly_salary) || null : null, daily_rate: form.staff_type === "daily" ? parseFloat(form.daily_rate) || null : null, date_hired: form.date_hired || null };
      if (editTarget) {
    ...
    } catch (e) { setAlert({ type: "error", msg: "Failed to save staff. " + e.message }); }
  ...
  const handleToggleActive = async (s) => {
    ...
    } catch (e) { setAlert({ type: "error", msg: "Failed to update status. " + e.message }); }
  };
    ...
    <div>
      ...
      {showForm && (
        <div style={{ ...styles.card, marginBottom: "24px", borderColor: theme.accent + "44" }}>
          ...
          <div style={styles.grid(3)}>
            <div style={styles.formGroup}><label style={styles.label}>Full Name *</label><input style={styles.input} placeholder="e.g. Emeka Okafor" value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} /></div>
            <div style={styles.formGroup}><label style={styles.label}>Phone Number</label><input style={styles.input} placeholder="+234..." value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} /></div>
            <div style={styles.formGroup}><label style={styles.label}>Role</label><select style={styles.input} value={form.role} onChange={e => setForm({ ...form, role: e.target.value })}>{ROLES.map(r => <option key={r}>{r}</option>)}</select></div>
            <div style={styles.formGroup}><label style={styles.label}>Staff Type</label><select style={styles.input} value={form.staff_type} onChange={e => setForm({ ...form, staff_type: e.target.value })}><option value="permanent">Permanent</option><option value="daily">Daily</option></select></div>
            {form.staff_type === "permanent" ? (
              <div style={styles.formGroup}><label style={styles.label}>Monthly Salary (₦)</label><input style={styles.input} type="number" placeholder="e.g. 85000" value={form.monthly_salary} onChange={e => setForm({ ...form, monthly_salary: e.target.value })} /></div>
            ) : (
              <div style={styles.formGroup}><label style={styles.label}>Daily Rate (₦)</label><input style={styles.input} type="number" placeholder="e.g. 3500" value={form.daily_rate} onChange={e => setForm({ ...form, daily_rate: e.target.value })} /></div>
            )}
            <div style={styles.formGroup}><label style={styles.label}>Date Hired</label><input style={styles.input} type="date" value={form.date_hired} onChange={e => setForm({ ...form, date_hired: e.target.value })} /></div>
          </div>
          <div style={styles.row}>
      ...
      <div style={styles.card}>
        ...
        {loading ? <Spinner /> : staff.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px", color: theme.textMuted, fontSize: "13px" }}>No staff records yet.</div>
        ) : (
          <table style={styles.table}>
            <thead><tr>{["Name","Role","Type","Pay Rate","Date Hired","Status","Actions"].map(h => <th key={h} style={styles.th}>{h}</th>)}</tr></thead>
            <tbody>
              {staff.map(s => (
                <tr key={s.id}>
...
const AttendanceTab = () => {
  ...
  const [rows, setRows] = useState([]); // { staff_id, full_name, role, staff_type, daily_rate, present, hours_worked, notes }
  const [saving, setSaving] = useState(false);
  const [alert, setAlert] = useState(null);
  ...
  useEffect(() => {
    ...
    attendanceService.getByDate(attendanceDate).then(existing => {
      ...
      setRows(activeStaff.map(s => {
        ...
        return { staff_id: s.id, full_name: s.full_name, role: s.role, staff_type: s.staff_type, daily_rate: s.daily_rate, present: ex ? ex.present : true, hours_worked: ex ? (ex.hours_worked || "") : "", notes: ex ? (ex.notes || "") : "" };
      }));
    }).catch(() => {});
  ...
  const loadHistory = async () => {
    setHistLoading(true);
    try {
      const data = await attendanceService.getByRange(histFrom, histTo, histStaff || null);
      setHistory(data);
    ...
    <div>
      ...
      {view === "entry" && (
        <div>
          ...
          {rows.length === 0 ? (
            <div style={{ ...styles.card, textAlign: "center", padding: "40px", color: theme.textMuted }}>No active staff found. Add staff in the Directory tab.</div>
          ) : (
            <div style={styles.card}>
              <div style={styles.sectionTitle}>Staff Attendance — {attendanceDate}</div>
              <table style={styles.table}>
                <thead>
                  <tr>{["Name","Role","Type","Present/Absent","Hours Worked","Notes"].map(h => <th key={h} style={styles.th}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {rows.map(r => (
      ...
      {view === "history" && (
        <div>
          ...
          {histLoading ? <Spinner /> : filteredHistory.length === 0 ? (
            <div style={{ ...styles.card, textAlign: "center", padding: "40px", color: theme.textMuted }}>No attendance records found. Try adjusting your filters.</div>
          ) : (
            <div style={styles.card}>
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      const lines = allStaff.map(s => {
        ...
        return { staff_id: s.id, full_name: s.full_name, role: s.role, staff_type: s.staff_type, days_present: daysPresent, daily_rate: s.daily_rate || 0, monthly_salary: s.monthly_salary || 0, amount_due: Math.round(amountDue) };
      });
      setCalcLines(lines);
      setStep(2);
                ...
                <tbody>
                  {runs.map(r => (
                    <tr key={r.id}>
                      ...
                      <td style={styles.td}>
                        <div style={{ display: "flex", gap: "6px" }}>
                          <button style={{ ...styles.btn("secondary"), padding: "4px 10px", fontSize: "11px" }} onClick={() => openRun(r)}>View</button>
                        </div>
                      </td>
                    </tr>
                  ))}
                ...
                {runLines.map(l => {
                  ...
                  return (
                    <tr key={l.id}>
                      ...
                      <td style={styles.td}>
                        {editable ? <input style={{ ...styles.input, width: "110px" }} type="number" value={e.amount_paid} onChange={ev => setPaymentEdits(pe => ({ ...pe, [l.id]: { ...pe[l.id], amount_paid: ev.target.value } }))} /> : naira(l.amount_paid)}
                      </td>
                      <td style={styles.td}>
                        {editable ? <input type="date" style={{ ...styles.input, width: "130px" }} value={e.payment_date} onChange={ev => setPaymentEdits(pe => ({ ...pe, [l.id]: { ...pe[l.id], payment_date: ev.target.value } }))} /> : l.payment_date || "—"}
                      </td>
                      <td style={styles.td}>
                        {editable ? <select style={{ ...styles.input, width: "110px" }} value={e.payment_method} onChange={ev => setPaymentEdits(pe => ({ ...pe, [l.id]: { ...pe[l.id], payment_method: ev.target.value } }))}><option value="cash">Cash</option><option value="transfer">Transfer</option></select> : l.payment_method || "—"}
                      </td>
    ...
    <div>
      <div style={styles.header}>
        <div>
          ...
          <div style={styles.pageSubtitle}>Directory, attendance tracking, and payroll</div>
        </div>
      </div>
      <div style={{ display: "flex", gap: "8px", marginBottom: "24px", borderBottom: `1px solid ${theme.border}`, paddingBottom: "12px" }}>
        {[["directory","Staff Directory"],["attendance","Attendance"],["payroll","Payroll"]].map(([id, label]) => (
